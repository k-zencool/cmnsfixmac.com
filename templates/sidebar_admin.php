        ...
        <div class="has-sub <?php echo $trackingOpen ? 'open' : ''; ?>">
            ...
            <div id="tracking-sub" class="submenu-wrapper <?php echo $trackingOpen ? 'open' : ''; ?>">
                <div class="submenu-inner">
                    ...
                    <a href="/admin/tracking/index.php?status=process" class="<?php echo has('status=process') ? 'sub-active' : ''; ?>">
                        <span class="material-symbols-rounded">handyman</span> กำลังซ่อม
                    </a>
                    <a href="/admin/tracking/index.php?status=done" class="<?php echo has('status=done') ? 'sub-active' : ''; ?>">
                        <span class="material-symbols-rounded">verified</span> รอรับ/เสร็จสิ้น
                    </a>
                    <a href="/admin/tracking/index.php?status=all" class="<?php echo has('status=all') ? 'sub-active' : ''; ?>">
...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    .sidebar { width: var(--sidebar-width); background: var(--sidebar-bg); height: 100vh; position: fixed; top: 0; left: 0; z-index: 1000; border-right: 1px solid var(--border-color); display: flex; flex-direction: column; transition: transform 0.3s ease; overflow-y: auto; }
    .sidebar-header { padding: 20px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid var(--border-color); flex-shrink: 0; }
    .sidebar-header .logo-area { display: flex; align-items: center; gap: 10px; color: var(--primary-color); }
    .sidebar-header h2 { margin: 0; font-size: 1.25rem; font-weight: 700; color: #111827; }
    .close-btn { background: none; border: none; font-size: 1.5rem; cursor: pointer; display: none; color: #6b7280; outline: none; }
    
    .sidebar-nav { padding: 15px 10px; flex: 1; }
    .sidebar-nav > a { display: flex; align-items: center; gap: 12px; padding: 12px 15px; color: var(--sidebar-text); text-decoration: none; border-radius: 8px; margin-bottom: 4px; transition: background-color 0.2s; font-size: 0.95rem; white-space: nowrap; }
    .sidebar-nav > a:hover { background-color: var(--sidebar-hover-bg); color: #111827; }
    .sidebar-nav > a.active { background-color: var(--primary-color); color: var(--primary-text); font-weight: 500; }
    .sidebar-nav a .material-symbols-rounded { font-size: 20px; flex-shrink: 0; }

    /* Dropdown Styles */
    .has-sub { margin-bottom: 4px; }
    .sub-head { display: flex; align-items: center; justify-content: space-between; border-radius: 8px; transition: background-color 0.2s; width: 100%; overflow: hidden; }
    .sub-head:hover { background-color: var(--sidebar-hover-bg); }

    .sub-link { flex: 1; display: flex; align-items: center; gap: 12px; padding: 12px 0 12px 15px; color: var(--sidebar-text); text-decoration: none; font-size: 0.95rem; white-space: nowrap; min-width: 0; outline: none; }
    .link-text { white-space: nowrap; }

    .sub-toggle { width: 40px; height: 44px; background: transparent; border: 0; cursor: pointer; display: flex; align-items: center; justify-content: center; color: #9ca3af; flex-shrink: 0; outline: none; }
    .sub-head:hover .sub-link { color: #111827; }
    .sub-head:hover .sub-toggle { color: #4b5563; }
    .sub-link.active-parent { color: var(--primary-color); font-weight: 600; }
    .sub-link.active-parent .material-symbols-rounded { color: var(--primary-color); }

    /* Animation */
    .has-sub.open .sub-toggle .material-symbols-rounded { transform: rotate(180deg); transition: transform 0.3s; }
    ...
    @media (min-width: 769px) {
        .has-sub:hover .submenu-wrapper { grid-template-rows: 1fr; }
        .has-sub:hover .sub-toggle .material-symbols-rounded { transform: rotate(180deg); }
        .has-sub:hover .sub-head { background-color: var(--sidebar-hover-bg); color: #111827; }
    }

    .submenu-inner { overflow: hidden; padding-left: 12px; }
    .submenu-inner a { display: flex; align-items: center; gap: 8px; padding: 10px 15px; font-size: 0.9rem; color: #6b7280; text-decoration: none; border-radius: 8px; margin-bottom: 2px; white-space: nowrap; }
    .submenu-inner a:hover { color: var(--primary-color); background: rgba(37, 99, 235, 0.05); }
    .submenu-inner a.sub-active { color: var(--primary-color); background: rgba(37, 99, 235, 0.1); font-weight: 600; }

    /* Mobile */
    .menu-btn { position: fixed; top: 15px; left: 15px; z-index: 999; background: #fff; border: 1px solid #e5e7eb; border-radius: 8px; padding: 8px; cursor: pointer; display: none; box-shadow: 0 2px 5px rgba(0,0,0,0.05); }
    .overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 998; display: none; opacity: 0; transition: opacity 0.3s; }
