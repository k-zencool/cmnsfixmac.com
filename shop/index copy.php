    ...
    <ul class="cmnsx-grid">
      ...
      <li class="cmnsx-card">
        ...
        <div class="cmnsx-info">
          ...
          <?php
            $line_oa   = '@cmnsfixmac';
            $prod_url  = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https://' : 'http://')
              . $_SERVER['HTTP_HOST'] . $url;
            $line_text = "สั่งซื้อสินค้า: {$name}\nราคา: " . number_format($price, 0) . " บาท\n" . $prod_url;
            $line_url  = 'https://line.me/R/ti/p/' . rawurlencode($line_oa) . '?text=' . rawurlencode($line_text);
          ?>
...
<head>
  ...
  <link rel="stylesheet" href="/shop/assets/css/card-actions.css"><!-- ปุ่ม LINE + ตะกร้าในการ์ด -->
...
<aside id="receipt-modal" role="dialog" aria-modal="true" aria-labelledby="receipt-title" style="display:none;">
  ...
  <div class="rcp-body">
    ...
    <div class="rcp-bank">
      ...
      <p>ชื่อบัญชี: <b>CMNS FixMac</b><br>ธ.กสิกรไทย <b>123-4-56789-0</b><br>พร้อมเพย์ <b>08x-xxx-xxxx</b></p>
      <small>โอนแล้วส่งสลิปยืนยันที่ LINE: @cmnsfixmac</small>
    </div>
  </div>
  ...
  <footer class="rcp-foot">
    ...
    <a id="rcp-line" class="rcp-btn line" href="#" target="_blank" rel="noopener">
      <span class="material-symbols-rounded">chat_bubble</span> ส่งไป LINE
    </a>
...
<style>
/* ===== Mini Cart (inline minimal for completeness) ===== */
...
#receipt-modal > *{ width:min(720px, 96vw); }
...
.rcp-body{ padding:14px 16px; max-height:62vh; overflow:auto; }
...
.rcp-bank{ margin-top:12px; background:#f9fafb; border:1px dashed #cbd5e1; padding:10px; border-radius:12px; }
...
@media(max-width:520px){ .rcp-item{ grid-template-columns:48px 1fr auto; } .rcp-thumb{ width:48px; height:48px; } }

/* Extra alignment for cart icon on price row */
...
(function(){
  ...
  function openCart(){ overlay.style.display='block'; drawer.style.display='flex'; drawer.focus(); }
  function closeCart(){ overlay.style.display='none'; drawer.style.display='none'; }

  if (navCart){
    navCart.addEventListener('click', function(e){ e.preventDefault(); openCart(); });
    navCart.addEventListener('keydown', function(e){
  ...
  // Add-to-cart (delegate on document)
  document.addEventListener('click', function(e){
    ...
    setTimeout(()=>{ btn.classList.remove('added'); btn.disabled=false; }, 350);
  });

  // Qty +/- and remove in drawer
  ...
  function buildSummaryText(cart, total){
    ...
    rows.push(`\nชำระ: CMNS FixMac (กสิกร 123-4-56789-0) / พร้อมเพย์ 08x-xxx-xxxx`);
    return rows.join('\n');
  }
  ...
  function openReceipt(){
    ...
    rCopy.onclick = async ()=> {
      try { await navigator.clipboard.writeText(text); rCopy.textContent='คัดลอกแล้ว'; setTimeout(()=>rCopy.innerHTML='<span class="material-symbols-rounded">content_copy</span> คัดลอกรายการ',1200); }
      catch(e){ alert('คัดลอกไม่สำเร็จ'); }
    };
    rLine.href = 'https://line.me/R/ti/p/@cmnsfixmac?text=' + encodeURIComponent(text);
