...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $stmt = $pdo->prepare("UPDATE products SET name=?, price=?, category=?, productdetails=?, main_image=?, status=? WHERE id=?");
  $stmt->execute([$name, $price, $category, $details, $main_image, $status, $id]);
...
<main class="main" id="main-content">
  ...
  <form action="" method="POST" enctype="multipart/form-data" class="form-section">
    ...
    <textarea name="productdetails" class="rich-text"><?= htmlspecialchars($product['productdetails']) ?></textarea>

    <label>รูปหลัก:</label>
    <input type="file" name="main_image" accept="image/*" onchange="previewMainImage(event)">
    <div id="mainImagePreview">
      <?php if ($product['main_image']): ?>
        <img src="../../uploads/<?= $product['main_image'] ?>" class="preview-thumb">
      <?php endif; ?>
    </div>
    ...
    <!-- รูปเดิมเพิ่มเติม -->
      <?php foreach ($images as $img): ?>
        <div class="image-wrapper">
          <img src="../../uploads/<?= $img['image'] ?>" class="preview-thumb" />
          <a href="delete_image.php?id=<?= $img['id'] ?>&product_id=<?= $id ?>" class="remove-btn" onclick="return confirm('ลบรูปนี้ใช่ไหม?')">✕</a>
        </div>
      <?php endforeach; ?>
