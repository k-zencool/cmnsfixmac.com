...
<style>
  ...
  .gallery-item-box.is-deleted { opacity: 0.5; background: #ffebee; border-color: #ffcdd2; }
  .gallery-item-box.is-deleted::after { content: "เตรียมลบรายการนี้"; position: absolute; top:50%; left:50%; transform:translate(-50%, -50%); color: #c00; font-weight:bold; font-size: 1.2rem; background:rgba(255,255,255,0.8); padding:5px 10px; border-radius:4px;}
  ...
  .btn-replace-img { background: #e3f2fd; color: #1565c0; border: 1px solid #bbdefb; padding: 4px 12px; border-radius: 4px; font-size: 0.85rem; cursor: pointer; display: inline-block; margin-top:5px;}
  .btn-replace-img:hover { background: #bbdefb; }
  ...
  .msg-success { padding: 15px; border-radius: 5px; margin-bottom: 20px; background: #e8f5e9; color: #2e7d32; border: 1px solid #2e7d32; }
</style>
...
<div id="loadingOverlay" class="loading-overlay"><div class="loading-spinner"></div><p style="color:white">กำลังบันทึกข้อมูล...</p></div>
...
function previewReplace(input, id) {
    if(input.files && input.files[0]) {
        ...
        if(err) { myAlert(err); input.value = ''; return; }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    mDrop.ondrop = e => {
        ...
        if(e.dataTransfer.files.length) {
            ...
            if(err) { mInput.value=''; myAlert(err); return; }
    ...
    document.getElementById('add_gallery_btn').onclick = () => {
        ...
        drop.ondrop = e => {
            ...
            if(e.dataTransfer.files.length) {
                ...
                if(err) { input.value=''; myAlert(err); return; }
